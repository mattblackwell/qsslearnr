...
- Call the `ggplot` function using the `g4math` is the numerical variable and `classtype` is the categorical variable. Pass the `star` data frame to the `data` argument. Include informative x-axis and y-axis labels with `labs` function. Finally, use the `rename` function to give the class types more intuitive names. 

pass the `classnames` vector to the `names` argument to place labels on the boxplots.
...
ggplot(star, aes(x = classtype, y = g4math)) + 
              geom_boxplot() +
              labs(y = "Math Scores", x = "Classtype")
```
