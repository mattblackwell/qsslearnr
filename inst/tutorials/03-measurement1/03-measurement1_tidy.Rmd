---
title: "Tidyverse version of QSS Tutorial 3: Measurment I"
output:
  learnr::tutorial:
    progressive: true
    ace_theme: github
...
- Use the `str` function on the `star` to see what the data looks like. 
...
- Use the `glimpse` function on the `star` to see what the data looks like. `glimpse` is particularly useful when the data set contains a long list of variables (columns), as it allows you see every column in the data set. 
...
#### Exercises
- Use the `drop_na` functions to drop the rows with explicit missing values in variable `g4math`. However, it is always vital to make sure that you are not dropping the observations that are important in other representations of the data.
...
## make a barplot with ggplot
ggplot(classcounts, aes(x = classtype, y = n)) +
...
ggplot(classcounts, aes(x=factor(classtype), y = n)) +
  geom_bar(stat='identity') + 
...
#### Exercises
- With `ggplot()`, use the `geom_histogram()` argument to create a histogram for the `g4math` variable in the `star` data frame.
...
Same as the barplot, there are several arguments you can pass to `ggplot()` that will improve its readability:
...
We'll often want to add more information to a plot to make it even more readable. You can do that with geoms that add to the current plot, such as `geom_abline` and `text`.
...
ggplot(star, aes(x = g4math)) +
  ...
  labs(x = "Score",
       title = "Distribution of fourth-grade math scores") +
  geom_vline(data = star, aes(xintercept = mean))
