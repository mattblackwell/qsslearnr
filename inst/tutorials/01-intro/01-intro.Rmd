---
title: "QSS Tutorial Week 0: Introduction to R"
output:
  learnr::tutorial:
    progressive: true
runtime: shiny_prerendered
description: >
  This tutorial will introduce you to the basics of R. 
---
...
tut_reptitle <- "QSS Tutorial Week 0: Output Report"
```
...
```{r divide-check}
grade_result(
  pass_if(~ identical(.result, 6/2), "Good job!")
)
```
...
```{r sqrt-check}
grade_result(
  pass_if(~ identical(.result, sqrt(16)), "Good job!")
)
```
...
```{r strings-check}
grade_result(
  pass_if(~ identical(.result, "learning R"), "Great job!")
)
```
