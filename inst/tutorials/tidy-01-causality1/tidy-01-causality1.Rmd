...
- Using the `count` and `pivot_wider` functions, create a cross tab of the `sex` and `call` variables in the resume data. 
...
- You can also give the mean a name directly in the summarize function by assigning a name to the `mean(call)` argument with `=`. Name the mean of the `call` variable as `callmean` and save this output as `resume.wf.callback`.


```{r name, exercise = TRUE}
## Name mean(call) directly in the summarize() function
...
- Calculate the mean of the `call` variable in the white-sounding name subset. Save this output as `resume.wf.callback`.
...
## calculate the mean of the callback variable of black-sounding names
  
## calculate the mean of the callback variable of white-sounding names
