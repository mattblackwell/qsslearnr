...
```{r factor-setup}
resume$carrie <- if_else(resume$firstname == "Carrie", 1, 0)
```
...
- Use the `group_by()` and `count()` functions on the `type` variable to see how many fictitious applications were sent out with each type of name.
...
- Use the `group_by()`, `select()`, and `summarize()` functions to calculate the `mean` of the `call` variable in each level of `race` and `sex`.
