...
- Use the `group_by()` and `count()` functions on the `type` variable  in `resume_type` data frame to see how many fictitious applications were sent out with each type of name.
...
- Use the `group_by()`, `select()`, and `summarize()` functions to calculate the `mean` of the `call` variable in each level of `race` and `sex` in the `resume_type` data frame.
...
```{r tapply-hint}
## use the `group_by()`, `select()`, and `summarize()` functions to calculate the mean in each level of race and sex
firstname_callback <- resume_type %>%
  group_by(sex, race) %>%
  select(???) %>%
  summarize(callback = ???)
firstname_callback
